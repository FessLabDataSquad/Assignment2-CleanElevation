...
names(study4) = variables_study4[, 1] #use short names for now; column 2 has the full items if i need them later
...
```{r}
study1$charity=rowMeans(study1[,c("optin","optout")],na.rm=TRUE) # this is goofy but it works
```
...
study3$height[study3$height==48]=NA #extreme value
...
```{r}
study1$age[study1$age==300]=NA #extreme value
...
```{r}
as.character(subset(study2, study2$playback==3)$playback.problems)
```
